...
/** An interactive building: front-bottom-center (cx, by) + footprint w. */
...
/** A decorative cottage (no `act` / `ico`). */
...
export const BLD: Record<string, Building> = {
  market: {
    act: "market",
    ico: "🏪",
    cx: 250,
...
export function plotPos(i: number): { x: number; y: number } {
  ...
  return {
    x: FIELD.startX + col * (FIELD.tile + FIELD.gap),
    y: FIELD.startY + row * (FIELD.tile + FIELD.gap),
  };
}
...
export function geom(b: Building): Geom {
  const w = b.w;
  const cx = b.cx;
  const by = b.by;
  const wh = w * 0.5;
...
function svgWindow(x: number, y: number, s: number): string {
  return (
    `<rect x="${nf(x)}" y="${nf(y)}" width="${nf(s)}" height="${nf(s * 0.9)}` +
...
function svgHouse(b: Building | Cottage): string {
  const g = geom(b as Building);
  let s = "";
  s += `<ellipse cx="${nf(b.cx + g.dx * 0.4)}" cy="${nf(b.by + g.w * 0.02)}" rx="${nf(g.w * 0.66)}" ry="${nf(g.w * 0.15)}" fill="rgba(0,0,0,0.16)"/>`;
  s += `<path d="M${nf(g.R)} ${nf(g.top)} L${nf(g.R + g.dx)} ${nf(g.top + g.dy)} L${nf(g.R + g.dx)} ${nf(g.by + g.dy)} L${nf(g.R)} ${nf(g.by)}Z" fill="${b.wall2}"/>`;
  ...
  const ww = g.w * 0.16;
  const wy = g.top + g.wh * 0.22;
  s += svgWindow(g.L + g.w * 0.13, wy, ww) + svgWindow(g.R - g.w * 0.13 - ww, wy, ww);
  const dw = g.w * 0.2;
  const dh = g.wh * 0.5;
  s += `<rect x="${nf(b.cx - dw / 2)}" y="${nf(g.by - dh)}" width="${nf(dw)}" height="${nf(dh)}" rx="${nf(dw * 0.28)}" fill="rgba(86,52,24,0.6)"/>`;
  s += `<path d="M${nf(b.cx)} ${nf(g.ridge)} L${nf(b.cx + g.dx)} ${nf(g.ridge + g.dy)} L${nf(g.R + g.e + g.dx)} ${nf(g.top + g.dy)} L${nf(g.R + g.e)} ${nf(g.top)}Z" fill="${b.roof2}"/>`;
  s += `<path d="M${nf(g.L - g.e)} ${nf(g.top)} L${nf(b.cx)} ${nf(g.ridge)} L${nf(g.R + g.e)} ${nf(g.top)}Z" fill="${b.roof}"/>`;
  s += `<path d="M${nf(b.cx)} ${nf(g.ridge)} L${nf(b.cx + g.dx)} ${nf(g.ridge + g.dy)}" stroke="rgba(255,255,255,0.4)" stroke-width="${nf(g.w * 0.03)}" stroke-linecap="round"/>`;
  return s;
}
...
function svgCloud(x: number, y: number, sc: number): string {
  const r = 8 * sc;
  return (
...
function svgPond(cx: number, cy: number, rx: number, ry: number): string {
  return (
    `<ellipse cx="${nf(cx)}" cy="${nf(cy + ry * 0.18)}" rx="${nf(rx * 1.05)}" ry="${nf(ry * 1.05)}" fill="rgba(0,0,0,0.12)"/>` +
...
function svgField(cx: number, cy: number, w: number, h: number, rot: number): string {
  const x = cx - w / 2;
  const y = cy - h / 2;
  return (
    `<g transform="rotate(${rot} ${nf(cx)} ${nf(cy)})">` +
...
function svgFence(x1: number, x2: number, y: number): string {
  let s =
    `<line x1="${nf(x1)}" y1="${nf(y)}" x2="${nf(x2)}" y2="${nf(y)}" stroke="#c9a26a" stroke-width="5"/>` +
    `<line x1="${nf(x1)}" y1="${nf(y - 9)}" x2="${nf(x2)}" y2="${nf(y - 9)}" stroke="#dab57e" stroke-width="4"/>`;
  for (let x = x1; x <= x2 + 0.1; x += 30)
    s += `<rect x="${nf(x - 2.5)}" y="${nf(y - 16)}" width="5" height="23" rx="2" fill="#b98e58"/>`;
  return s;
}

function svgPenRect(L: number, R: number, T: number, B: number): string {
  let s =
    `<rect x="${nf(L)}" y="${nf(T)}" width="${nf(R - L)}" height="${nf(B - T)}" rx="14" fill="#a9d57e"/>` +
...
function svgPens(): string {
  let s = "";
  PEN_DEFS.forEach((pn) => {
    s += svgPenRect(pn.L, pn.R, pn.T, pn.B);
  });
  return s;
}

// The tilled backing + fence behind the playable crop grid.
function svgFieldBacking(): string {
  const x = FIELD.startX - 24;
  const y = FIELD.startY - 24;
  const w = FIELD.fullW + 48;
  const h = FIELD.fullH + 48;
  let s = `<rect x="${nf(x - 4)}" y="${nf(y - 4)}" width="${nf(w + 8)}" height="${nf(h + 8)}" rx="20" fill="#6f9c45"/>`;
...
const PATH_D = "M600 1082 C 560 940 700 800 600 640 S 500 460 600 320";
...
export function buildScene(): string {
  ...
  s +=
    svgCloud(560, 70, 3.4) +
    svgCloud(880, 100, 2.6) +
    svgCloud(330, 110, 2.2) +
    svgCloud(1080, 60, 2.8);
