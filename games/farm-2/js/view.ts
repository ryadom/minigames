...
function soilCell(i: number, t: Tile): string {
  ...
  return (
    `<button class="${cls}" aria-label="${esc(tf("soilLabel", { n: i + 1 }))}: ${esc(t.crop ? name(t.crop) : MG.i18n.t("emptyLabel"))}" data-plotcell="${i}" data-stage="${stage}" data-act="plot" data-arg="${i}" ${cellStyle(i)}>` +
...
function buildCell(i: number): string {
  ...
  return (
    ...
    `aria-label="${esc(tf("cellLabel", { n: i + 1 }))}: ${esc(t ? (t.kind === "soil" ? MG.i18n.t("bSoil") : buildName(BUILD_BY_ID[t.kind === "pen" ? `pen-${t.penType}` : t.kind])) : "+")}" data-act="buildcell" data-arg="${i}" ${cellStyle(i, cw, ch)}>${inner}</button>`
  );
}
...
/* ----------------------------- TOOLBARS ------------------------------ */
// Keep usable seeds and tools ahead of crops that still need to be unlocked.
function renderToolbar(): string {
  ...
  const seedButton = (c: (typeof CROPS)[number]): string => {
  ...
  h += CROPS.filter((c) => isUnlocked(c.lvl))
    .map(seedButton)
    .join("");
  h +=
  ...
  return (
    h +
    CROPS.filter((c) => !isUnlocked(c.lvl))
      .map(seedButton)
      .join("")
  );
}
...
export function render(): void {
  const focused =
    document.activeElement instanceof HTMLElement && document.activeElement.dataset.act
      ? { act: document.activeElement.dataset.act, arg: document.activeElement.dataset.arg }
      : null;
  ensureScale();
  ...
  syncStats();
  if (focused) {
    const match = Array.from(document.querySelectorAll<HTMLButtonElement>("button[data-act]")).find(
      (el) => el.dataset.act === focused.act && el.dataset.arg === focused.arg && !el.disabled,
    );
...
function renderChrome(): void {
  ...
  text("farmName", MG.i18n.t("farmName"));
  text("farmSubtitle", MG.i18n.t("subtitle"));
  const mode = state.build ? (state.buildSel === MOVE_TOOL ? "move" : "build") : "farm";
  const modes = document.getElementById("modebar");
  if (modes)
    modes.innerHTML = [
      ["farm", "🌱", "playMode"],
      ["build", "🔨", "buildMode"],
      ...
      .map(
        ([id, icon, label]) =>
          `<button type="button" data-act="game-mode" data-arg="${id}" aria-pressed="${mode === id}">${icon} ${esc(MG.i18n.t(label))}</button>`,
      )
      .join("");
  text(
    "buildHint",
  ...
  const catalog = document.getElementById("catalogTitle");
  if (catalog)
    catalog.innerHTML =
      `<span>${esc(MG.i18n.t(state.build ? "buildCatalog" : "seedCatalog"))}</span>` +
      (state.placeAt != null
        ? `<span><button class="btn go" data-act="placeok">${esc(MG.i18n.t("confirmBuild"))}</button> <button class="btn alt" data-act="placecancel">${esc(MG.i18n.t("cancelBuild"))}</button></span>`
        : "");
  const quick = document.getElementById("quickbar");
  if (quick)
    quick.innerHTML = [
      ["market", "open", "market", "🏪", "tabMarket"],
      ["kitchen", "open", "cook", "🍳", "tabCook"],
      ["pen", "openpen", "chicken", "🐔", "chicken"],
      ["board", "open", "quests", "📋", "tabQuests"],
      ["storage", "open", "storage", "📦", "tabStorage"],
      ["research", "open", "research", "🔬", "tabResearch"],
    ]
      .map(
        ([kind, act, arg, icon, label]) =>
          `<button type="button" data-act="${act}" data-arg="${arg}" ${state.grid.some((t) => t?.kind === kind && (kind !== "pen" || t.penType === arg)) ? "" : "disabled"}>${icon} ${esc(kind === "pen" ? name(label) : MG.i18n.t(label))}</button>`,
      )
      .join("");
}
