  ...
  actions: [
    {
      ...
      labelKey: "help",
      onClick: () => {
        $("helpTitle").textContent = MG.i18n.t("helpTitle");
        $("helpBody").textContent = MG.i18n.t("helpBody");
        $("helpClose").textContent = MG.i18n.t("modeDone");
        (document.getElementById("helpDialog") as HTMLDialogElement).showModal();
      },
    },
    {
      key: "new",
...
document.getElementById("farmChrome")?.addEventListener("click", (event) => {
  const button = (event.target as Element).closest<HTMLButtonElement>("button[data-act]");
  if (button && !button.disabled) handle(button.dataset.act as string, button.dataset.arg);
});
function renderMapControls(): void {
  $("mapControls").innerHTML = [
    ...
    .map(
      ([id, icon, key]) =>
        `<button type="button" data-zoom="${id}" aria-label="${MG.i18n.t(key)}" title="${MG.i18n.t(key)}">${icon}</button>`,
    )
    .join("");
}
$("mapControls").addEventListener("click", (event) => {
  const button = (event.target as Element).closest<HTMLButtonElement>("button[data-zoom]");
  if (button)
    button.dataset.zoom === "fit" ? fitMap() : zoomMap(button.dataset.zoom === "in" ? 1 : -1);
});
document.addEventListener("keydown", (event) => {
  if (
    ...
    event.metaKey ||
    event.altKey ||
    (document.getElementById("helpDialog") as HTMLDialogElement).open
  )
    return;
  if ((event.target as HTMLElement).matches("input, select, textarea, [contenteditable=true]"))
    return;
  const key = event.key.toLowerCase();
  if (key === "b" || key === "m" || key === "escape") {
    event.preventDefault();
    handle("game-mode", key === "b" ? "build" : key === "m" ? "move" : "farm");
  } else if (key === "+" || key === "=" || key === "-") zoomMap(key === "-" ? -1 : 1);
  else if (key === "0") fitMap();
});
